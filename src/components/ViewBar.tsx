...
export function ViewBar({ onHome, onFit, onZoomIn, onZoomOut, onFullscreen, fullscreen }: Props) {
  ...
  return (
    <div className="viewbar" onMouseDown={(e) => e.stopPropagation()}>
      {btn("Acasă (model principal)", onHome, svg(<><path d="M3 11l9-8 9 8" /><path d="M5 9v11h14V9" /></>))}
      {btn("Încadrează tot", onFit, svg(<><path d="M4 9V4h5M20 9V4h-5M4 15v5h5M20 15v5h-5" /></>))}
      <span className="viewbar-sep" />
      {btn("Mărește", onZoomIn, svg(<><circle cx="11" cy="11" r="7" /><path d="M21 21l-4.3-4.3M11 8v6M8 11h6" /></>))}
      {btn("Micșorează", onZoomOut, svg(<><circle cx="11" cy="11" r="7" /><path d="M21 21l-4.3-4.3M8 11h6" /></>))}
      <span className="viewbar-sep" />
      {btn(fullscreen ? "Ieși din ecran complet" : "Ecran complet", onFullscreen,
        fullscreen
